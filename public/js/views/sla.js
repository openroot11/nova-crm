...
export async function mount(container, ctx) {
  container.innerHTML = `
    <div class="mb-gutter flex justify-between items-end flex-wrap gap-gutter">
      <div>
        <h2 class="text-headline-lg font-headline-lg text-on-surface mb-base">Control SLA 24h</h2>
        <p class="text-body-md font-body-md text-on-surface-variant">Listado de clientes críticos: 24h sin contacto o 24h contactados sin cotización.</p>
      </div>
    ...
    <div class="bg-surface rounded-xl border border-outline-variant overflow-hidden shadow-sm">
      ...
      <div id="sla-loadmore-wrap" class="hidden p-4 border-t border-outline-variant text-center">
        <button id="sla-loadmore-btn" class="px-4 py-2 rounded-lg border border-outline-variant text-body-sm font-label-bold text-on-surface hover:bg-surface-container-low transition-colors">Mostrar más</button>
  ...
  function funnelButtons(lead) {
    ...
    if (lead.status === 'asignado') {
      buttons.push(`<button data-id="${lead.id}" class="contact-btn bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant px-3 py-2 rounded-lg font-bold text-body-sm font-body-sm transition-colors">Contactado</button>`);
    }
    if (lead.status === 'contactado') {
      // Pospone el recordatorio 24h en vez de reasignar de una: el asesor
      // sigue en conversacion con el cliente, solo aun no llega a cotizar.
      buttons.push(`<button data-id="${lead.id}" class="still-contact-btn bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant px-3 py-2 rounded-lg font-bold text-body-sm font-body-sm transition-colors">Aún en contacto</button>`);
    }
    if (lead.status === 'asignado' || lead.status === 'contactado') {
      buttons.push(`<button data-id="${lead.id}" class="quote-btn bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant px-3 py-2 rounded-lg font-bold text-body-sm font-body-sm transition-colors">Cotizado</button>`);
    }
    return buttons.join('');
  }
  ...
  function rowHtml(lead) {
    ...
    return `
      <tr class="${vencido ? 'bg-error/5 hover:bg-error/10' : 'hover:bg-surface-container-low'} transition-colors">
        <td class="p-table-cell-padding font-bold"><div class="flex items-center gap-1.5">${escapeHtml(lead.client_name)}${copyNameBtn(lead.client_name)}</div></td>
        ...
        <td class="p-table-cell-padding text-right">
          <div class="flex flex-col items-end gap-2">
            ${lead.sla_reason === 'cotizacion' ? `<p class="text-body-sm font-bold text-tertiary text-right">Recordar a ${escapeHtml(lead.advisor_name || 'el asesor asignado')}: falta enviar cotización</p>` : ''}
            <div class="flex justify-end flex-wrap gap-2">${funnelButtons(lead)}</div>
            ${canReassignLead(ctx, lead) ? `
            <div class="flex justify-end">
              <button data-id="${lead.id}" class="reassign-btn ${vencido ? 'bg-error text-on-error hover:bg-error/90 shadow-sm' : 'bg-surface-container-highest text-on-surface hover:bg-surface-container-low border border-outline-variant'} px-4 py-2 rounded-lg font-bold text-body-sm font-body-sm transition-colors inline-flex justify-center items-center gap-2 whitespace-nowrap">
  ...
  tbody.addEventListener('click', (e) => {
    ...
    if (stillContactBtn) {
      const lead = critical.find((l) => l.id === Number(stillContactBtn.dataset.id));
      if (lead) markStillInContact(lead, ctx, load);
    }
  });
