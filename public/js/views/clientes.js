...
function fieldHtml(id, label, value, placeholder = '') {
  return `
    <div>
      ...
      <input id="${id}" type="text" value="${escapeHtml(value || '')}" placeholder="${placeholder}" class="w-full p-2.5 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline focus:ring-2 focus:ring-outline/20" />
    </div>
  `;
}
...
export async function mount(container, ctx) {
    ...
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl shadow-sm overflow-hidden mb-gutter">
      <div class="p-4 border-b border-outline-variant flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div class="ml-auto flex flex-wrap items-center gap-2">
          ...
          <button id="clientes-refresh-btn" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-outline-variant bg-surface text-body-sm font-label-bold text-on-surface hover:bg-surface-container-low transition">
      ...
      <div class="p-4 border-b border-outline-variant bg-surface-container-low flex flex-wrap gap-3 items-end">
        <div class="min-w-[200px] flex-1">
          ...
          <input id="filter-q" type="text" placeholder="Nombre, teléfono o documento..." class="w-full p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
        </div>
        ...
        <div>
          ...
          <select id="filter-compras" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline">
            <option value="">Todos</option>
        ...
        <div>
          ...
          <select id="filter-saldo" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline">
            <option value="">Todos</option>
        ...
        <div>
          ...
          <input id="filter-from" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
        </div>
        <div>
          <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Hasta</label>
          <input id="filter-to" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
        </div>
      ...
      <div id="clientes-loadmore-wrap" class="hidden p-4 border-t border-outline-variant text-center">
        <button id="clientes-loadmore-btn" class="px-4 py-2 rounded-lg border border-outline-variant text-body-sm font-label-bold text-on-surface hover:bg-surface-container-low transition-colors">Mostrar más</button>
        <p id="clientes-loadmore-info" class="text-[11px] text-on-surface-variant mt-1"></p>
      </div>
    </div>

    <div id="ficha-cliente" class="hidden border-2 border-outline/30 rounded-xl p-gutter shadow-sm bg-surface"></div>
  `;
  ...
  function rowHtml(c) {
    return `
      <tr class="hover:bg-surface-container-low/50 transition-colors cursor-pointer" data-open="${c.id}">
        <td class="p-table-cell-padding text-body-md font-semibold text-on-surface"><div class="flex items-center gap-1.5">${escapeHtml(c.name)}${copyNameBtn(c.name)}</div></td>
        ...
        <td class="p-table-cell-padding text-right">
          <button data-open="${c.id}" class="text-on-surface hover:text-on-primary-fixed-variant text-body-sm font-label-bold inline-flex items-center gap-1">
    ...
    openModal({
      ...
      render: (body, { close }) => {
        ...
        okBtn.addEventListener('click', async () => {
          ...
          const email = body.querySelector('#new-client-email').value.trim();
          const address = body.querySelector('#new-client-address').value.trim();
          ...
          try {
            okBtn.disabled = true;
            const newClient = await ctx.api.post('/api/clients', { name, phone, document, email, address, notes });
  ...
  function leadRowHtml(lead, payments) {
    ...
    const montoCell = canEditAmount
      ? `<button data-edit-amount="${lead.id}" class="inline-flex items-center gap-1 hover:text-on-surface transition-colors group">
    ...
    return `
      <tr class="hover:bg-surface-container-low/50 transition-colors">
        ...
        <td class="p-2 text-right">${canPay ? `<button data-pay="${lead.id}" class="text-on-surface hover:text-on-primary-fixed-variant text-[11px] font-label-bold border border-outline-variant rounded-md px-2 py-1">Registrar abono</button>` : ''}</td>
      </tr>
    `;
  }
  ...
  function paymentRowHtml(p, leadsById) {
    ...
    const actionsCell = canEditMoney
      ? `<td class="p-2 text-right whitespace-nowrap">
           <button data-edit-payment="${p.id}" data-lead-id="${p.lead_id}" class="text-on-surface-variant hover:text-on-surface p-1" title="Editar abono">
    ...
    openModal({
      ...
      render: (body, { close }) => {
        body.innerHTML = `
          ...
          <input id="pay-amount" type="number" min="1" step="1000" placeholder="0" class="w-full p-2.5 border border-outline-variant rounded-md mb-4 outline-none focus:border-outline focus:ring-2 focus:ring-outline/20" />
  ...
  function openEditPaymentModal(payment, onDone) {
    openModal({
      ...
      render: (body, { close }) => {
        body.innerHTML = `
          <label class="block text-label-bold font-label-bold uppercase tracking-wide text-on-surface-variant mb-1">Monto (COP)</label>
          <input id="pay-edit-amount" type="number" min="1" step="1000" value="${payment.amount}" class="w-full p-2.5 border border-outline-variant rounded-md mb-4 outline-none focus:border-outline focus:ring-2 focus:ring-outline/20" />
          <label class="block text-label-bold font-label-bold uppercase tracking-wide text-on-surface-variant mb-1">Nota (opcional)</label>
  ...
  async function openFicha(id) {
    ...
    fichaRoot.innerHTML = `
      <div class="flex flex-wrap items-start justify-between gap-3 mb-5">
        <div>
          <p class="text-[10px] font-label-bold text-on-surface uppercase tracking-wider mb-0.5">Ficha de cliente</p>
          <h3 class="text-headline-sm font-headline-sm text-on-surface flex items-center gap-1.5">${escapeHtml(client.name)}${copyNameBtn(client.name)}</h3>
        ...
        <button id="ficha-cerrar" class="p-2 text-on-surface-variant hover:text-on-surface transition-colors">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-gutter mb-gutter">
        ${fieldHtml('ficha-email', 'Correo electrónico', client.email, 'Opcional')}
        ${fieldHtml('ficha-address', 'Dirección', client.address, 'Opcional')}
      </div>
      <div class="mb-gutter">
        <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Notas</label>
        <textarea id="ficha-notes" rows="3" class="w-full p-2.5 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline focus:ring-2 focus:ring-outline/20">${escapeHtml(client.notes || '')}</textarea>
      </div>
      <button id="ficha-guardar" class="mb-gutter px-4 py-2 border border-outline-variant rounded-md text-label-bold font-label-bold text-on-surface hover:bg-surface-container-low transition-colors">Guardar datos</button>
    ...
    fichaRoot.querySelector('#ficha-guardar').addEventListener('click', async () => {
      try {
        await ctx.api.patch(`/api/clients/${id}`, {
          ...
          email: fichaRoot.querySelector('#ficha-email').value.trim(),
          address: fichaRoot.querySelector('#ficha-address').value.trim(),
