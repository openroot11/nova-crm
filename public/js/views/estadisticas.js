...
export async function mount(container, ctx) {
  container.innerHTML = `
    ...
    <div id="dashboard-card" class="bg-surface-container-lowest border border-outline-variant rounded-xl p-gutter shadow-sm mt-gutter">
      <div class="flex flex-wrap items-end justify-between gap-4 mb-6">
        ...
        <div class="flex items-end gap-3 flex-wrap no-print">
          <div>
            ...
            <select id="asesor-filter" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline">
              <option value="">General</option>
            </select>
          </div>
          <div>
            <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Desde</label>
            <input id="funnel-from" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
          </div>
          <div>
            <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Hasta</label>
            <input id="funnel-to" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
          </div>
          ...
          <div id="compare-dates" class="hidden flex items-end gap-3">
            <div>
              ...
              <input id="compare-from" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
            </div>
            <div>
              <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Comparar hasta</label>
              <input id="compare-to" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
            </div>
          </div>
      ...
      <div id="ficha-asesor" class="hidden border-2 border-outline/30 rounded-xl p-4 mb-gutter bg-surface">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
          <div>
            <p class="text-[10px] font-label-bold text-on-surface uppercase tracking-wider mb-0.5">Ficha individual</p>
    ...
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-gutter shadow-sm mt-gutter">
      <div class="flex flex-wrap items-end justify-between gap-4 mb-6">
        ...
        <div class="flex items-end gap-3 flex-wrap">
          <div>
            <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Desde</label>
            <input id="profit-from" type="date" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
          </div>
          <div>
            <label class="block text-[10px] font-label-bold text-on-surface-variant mb-1 uppercase tracking-wider">Hasta</label>
      ...
      <div class="flex flex-wrap items-end gap-3 mb-6 p-4 bg-surface-container-low rounded-lg border border-outline-variant">
        <div>
          ...
          <input id="spend-month" type="month" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline" />
        </div>
        <div>
          ...
          <input id="spend-amount" type="number" min="0" step="1000" placeholder="0" class="p-2 bg-surface-container-lowest border border-outline-variant rounded-md text-body-sm outline-none focus:border-outline w-40" />
        </div>
        <button id="spend-save-btn" class="px-3 py-2 border border-outline-variant rounded-md text-label-bold font-label-bold text-on-surface hover:bg-surface-container-lowest transition-colors">Guardar inversión</button>
      </div>
  ...
  async function load() {
    ...
    kpiGrid.innerHTML = `
      <div class="bg-surface-container-lowest border border-outline-variant rounded-xl p-gutter shadow-sm relative overflow-hidden">
        <div class="absolute top-0 right-0 p-4 opacity-10"><span class="material-symbols-outlined text-6xl text-on-surface">attach_money</span></div>
    ...
    groupedBarChart(ventasAsesorCanvas, {
      ...
      series: [
        { label: 'Mes Actual', data: k.sales_by_advisor.map((a) => a.this_month), color: '#981b1e' },
        { label: 'Mes Anterior', data: k.sales_by_advisor.map((a) => a.prev_month), color: '#d9a7a9' },
      ],
    ...
    return `
      <div class="border border-outline-variant rounded-xl p-3">
        <div class="flex items-center gap-1.5 mb-1">
          <span class="material-symbols-outlined text-[15px] text-on-surface">${icon}</span>
  ...
  function renderTabs() {
    metricTabs.innerHTML = METRICS.map(
      (m) =>
        `<button data-metric="${m.key}" class="metric-btn px-3 py-1 rounded transition-colors ${m.key === activeMetric ? 'bg-surface text-on-surface shadow-sm font-bold' : 'text-on-surface-variant'}">${m.label}</button>`
    ).join('');
  ...
  const canDeleteReport = ctx.user?.role === 'admin';
      ...
      <tr>
        ...
        <td class="p-table-cell-padding text-right">
          <div class="flex items-center justify-end gap-3">
            ...
            ${canDeleteReport ? `
            <button data-action="delete-report" data-id="${r.id}" class="inline-flex items-center gap-1 text-error hover:opacity-80 text-body-sm font-label-bold">
  ...
  reportsTbody.addEventListener('click', async (e) => {
    const btn = e.target.closest('button[data-action="delete-report"]');
    if (!btn) return;
    const ok = await confirmModal({
      title: 'Borrar reporte archivado',
      message: 'Esta foto fija del reporte se eliminará del historial. Esta acción no se puede deshacer.',
      confirmLabel: 'Borrar reporte',
      danger: true,
    });
    if (!ok) return;
    try {
      await ctx.api.del(`/api/reports/${btn.dataset.id}`);
