...
function funnelActionBtn(lead) {
  if (lead.status === 'asignado') {
    return `<button data-action="contact" data-id="${lead.id}" data-kind="sla" class="px-2.5 py-1.5 bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant rounded-md text-[11px] font-label-bold transition-colors">Contactado</button>`;
  }
  if (lead.status === 'contactado') {
    return `
      <button data-action="still-contact" data-id="${lead.id}" data-kind="sla" class="px-2.5 py-1.5 bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant rounded-md text-[11px] font-label-bold transition-colors">Aún en contacto</button>
      <button data-action="quote" data-id="${lead.id}" data-kind="sla" class="px-2.5 py-1.5 bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant rounded-md text-[11px] font-label-bold transition-colors">Cotizado</button>
    `;
  }
  return '';
}

function slaRowHtml(lead) {
  const vencido = lead.sla_status === 'vencido';
  const sinCotizacion = lead.sla_reason === 'cotizacion';
  return `
    <div class="flex items-center justify-between gap-3 px-3 py-2.5 ${vencido ? 'bg-error/5' : ''} rounded-lg">
      <div class="min-w-0 flex items-center gap-2.5">
        ...
        <div class="min-w-0">
          <p class="text-body-sm font-bold text-on-surface truncate flex items-center gap-1">${escapeHtml(lead.client_name)}${copyNameBtn(lead.client_name)}</p>
          <p class="text-[11px] text-on-surface-variant truncate">
            ${sinCotizacion ? `Recordar a ${escapeHtml(lead.advisor_name || 'el asesor asignado')}: falta cotización` : `${vencido ? 'Vencido' : 'En riesgo'} · ${escapeHtml(lead.remaining_label || '')} · ${escapeHtml(lead.advisor_name || 'Sin asignar')}`}
          </p>
        </div>
      </div>
      <div class="shrink-0 flex gap-1.5">${funnelActionBtn(lead)}</div>
    </div>
  `;
}
...
function followupRowHtml(lead) {
  ...
  return `
    <div class="flex items-center justify-between gap-3 px-3 py-2.5 rounded-lg">
      <div class="min-w-0 flex items-center gap-2.5">
        <span class="material-symbols-outlined text-[18px] ${urgente ? 'text-on-tertiary-fixed-variant' : 'text-tertiary'}">${urgente ? 'call' : 'schedule'}</span>
        <div class="min-w-0">
          <p class="text-body-sm font-bold text-on-surface truncate flex items-center gap-1">${escapeHtml(lead.client_name)}${copyNameBtn(lead.client_name)}</p>
          <p class="text-[11px] text-on-surface-variant truncate">${meta.label} · ${escapeHtml(lead.followup_elapsed_label || '')} · ${escapeHtml(lead.advisor_name || 'Sin asignar')}</p>
        </div>
      </div>
      <div class="shrink-0 flex gap-1.5">
        <button data-action="followup" data-id="${lead.id}" data-kind="followup" class="px-2.5 py-1.5 bg-surface-container-lowest text-on-surface hover:bg-surface-container-low border border-outline-variant rounded-md text-[11px] font-label-bold transition-colors">Seguir</button>
  ...
  return `
    <div>
      <div class="flex items-center justify-between mb-1.5">
        ...
        ${viewAllRoute ? `<button data-goto="${viewAllRoute}" class="text-[11px] font-label-bold text-on-surface hover:underline">Ver todo</button>` : ''}
      </div>
  ...
  root.innerHTML = `
    <div class="space-y-5">
      ${groupHtml(
        ...
        'timer',
        'text-error',
        critical.slice(0, MAX_ROWS_PER_GROUP).map((l) => slaRowHtml(l)),
