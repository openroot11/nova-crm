...
export async function mount(container, ctx) {
  container.innerHTML = `
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-gutter items-start">
      ...
      <div class="xl:col-span-8 bg-surface rounded-xl border border-outline-variant shadow-sm flex flex-col overflow-hidden">
    ...
    return `
      <tr class="hover:bg-surface-container-low/50 transition-colors">
        ...
        <td class="p-table-cell-padding">
          ...
          <div class="text-body-sm text-on-surface-variant">${escapeHtml(lead.product || '—')} · ${escapeHtml(lead.source || '—')}</div>
        </td>
  ...
  [filterAsesor, filterProducto, filterEstado, filterSource, filterFrom, filterTo].forEach((el) => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      const lead = await ctx.api.post('/api/leads', { client_name, phone, document: document_, advisor_id, product, source, notes });
