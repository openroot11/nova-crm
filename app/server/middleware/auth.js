...
async function devUser() {
  ...
  const real = await db.prepare("SELECT * FROM users WHERE role = 'admin' AND active = true ORDER BY id ASC LIMIT 1").get();
  return real || { id: null, username: 'dev', role: 'admin', advisor_id: null, active: true };
}
...
async function resolveUser(req) {
  ...
  const user = await db.prepare('SELECT * FROM users WHERE id = ? AND active = true').get(userId);
  return user || null;
}
