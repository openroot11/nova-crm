...
function withMetrics(advisor) {
  if (advisor.is_group) {
    return { ...advisor, close_rate: null, sla_rate: null, calls_today: null };
  }
  const leads = db.prepare('SELECT * FROM leads WHERE assigned_advisor_id = ?').all(advisor.id);
  const total = leads.length;
  const won = leads.filter((l) => l.status === 'cerrado_ganado').length;
  const slaOk = leads.filter((l) => sla.slaStatus(l) !== 'vencido').length;
...
router.get('/', (req, res) => {
  const advisors = rotation.getOrderedAdvisors().map(withMetrics);
  res.json(advisors);
});
...
router.post('/', requireRole('coordinador', 'admin'), (req, res) => {
  ...
  const info = db
    .prepare('INSERT INTO advisors (name, role, active, is_group, priority_order) VALUES (?, ?, 1, 0, ?)')
...
router.patch('/:id', requireRole('coordinador', 'admin'), (req, res) => {
  ...
  const { name, role } = req.body || {};
  db.prepare('UPDATE advisors SET name = COALESCE(?, name), role = COALESCE(?, role) WHERE id = ?').run(
    name ?? null,
    role ?? null,
    id
  );
