...
router.patch('/:id', async (req, res) => {
  ...
  const { username, role, advisor_id, active } = req.body || {};
  ...
  if (username !== undefined) {
    ...
    if (!cleanUsername) return res.status(400).json({ error: 'El usuario no puede quedar vacío' });
    const existing = await db.prepare('SELECT id FROM users WHERE lower(username) = lower(?) AND id != ?').get(cleanUsername, id);
    if (existing) return res.status(409).json({ error: 'Ya existe un usuario con ese nombre' });
  }

  await db
    .prepare('UPDATE users SET username = COALESCE(?, username), role = COALESCE(?, role), advisor_id = ?, active = COALESCE(?, active) WHERE id = ?')
    .run(
      cleanUsername ?? null,
      role ?? null,
