...
router.post('/', async (req, res) => {
  const { name, phone, document, notes, address, email } = req.body || {};
  ...
  const info = await db
    .prepare('INSERT INTO clients (name, phone, document, notes, address, email) VALUES (?, ?, ?, ?, ?, ?)')
    .run(
      name.trim(),
...
router.patch('/:id', async (req, res) => {
  ...
  await db
    .prepare('UPDATE clients SET name = ?, phone = ?, document = ?, notes = ?, address = ?, email = ? WHERE id = ?')
    .run(
      ...
      address !== undefined ? (address.trim() || null) : client.address,
      email !== undefined ? (email.trim() || null) : client.email,
      id
    );
